...
export default {
  ...
  methods: {
    async sendFormData() {
      try {
        const data = {name: this.name, number: this.phone,text:this.text};
        const res = await axios.post(`${API_URL}/api/mail/feedback`, data)
        const {data: {data: {success, message}}} = res;

        if (success) {
          this.$emit('modal'); // закрываем форму
          this.$emit('modalSuccess'); // вызываем модалку с успехом
        }
      } catch(e) {
        ...
        <div class="submit">
          <div class="submit_container">
            <div class="flex justify-between items-center">
              <h2 class="submit_title">письмо директору</h2>
              <button
                  @click="close"
